...
#        for row in invaders: # take the most left or right invader
        k = -1
        while not True in [i.alive for i in invaders[k]]: # find row with at least one invader alive
           k = k-1
        row = invaders[k]
        ...
        if inv.Moveable():# if above the 10
           if i_x <= 0 or i_x + inv.Shape()[1] >= width: # left & right side detection
              if i_x <= 0:
                 for row in invaders:
        ...
        for row in invaders: # draw on new position
           for inv in row:
              ...
              if inv.alive and inv.Shoot() and inv.bottom and inv.Moveable():
                 temp_proj = Projectile(0, 0, direction='down', speed=1, uprate=0.2,
                                        path=PROJECTIL_INV_PATH, color=inv.color)
